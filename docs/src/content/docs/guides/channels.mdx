...
Six files ship: `global`, `local`, `world`, `staff`, `party` and `server`. Only `global`
starts enabled. The rest are there so you can flip `enabled: true` on the ones you want
...
cross-server caveat are on the [integrations page](/integrations/overview/).
